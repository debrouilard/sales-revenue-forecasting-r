...
def forecast_arima(df: pd.DataFrame, periods=12) -> pd.DataFrame:
    model = auto_arima(df['Sales'], seasonal=True, m=12, trace=False)
    arima_model = ARIMA(df['Sales'], order=model.order, seasonal_order=model.seasonal_order)
    arima_result = arima_model.fit()
...
def forecast_ets(df: pd.DataFrame, periods=12) -> pd.Series:
    ets_model = ExponentialSmoothing(df['Sales'], seasonal='add', seasonal_periods=12).fit()
    forecast = ets_model.forecast(periods)
    return forecast
>>>>>>> add1225 (Add sales revenue forecasting pipeline)
